...
public strictfp class RobotPlayer {
		...
		while (true) {
			...
			try {
				...
				if (!inPlace) {
					...
					bigBoss = rc.getInitialArchonLocations(myTeam);

					Direction dir = null;

					// Move away from archons
					for (MapLocation ml : bigBoss) {
						if (ml.isWithinSensorRadius(rc.senseRobotAtLocation(rc.getLocation()))) {
							System.out.println("RUNNING FROM BIG BOSS");
							dir = rc.getLocation().directionTo(ml).opposite()
					...
					if (nearby != null) {
						...
						if (rc.canPlantTree(dir)) {
							System.out.println("I HAVE FOUND MY PLACE AND PLANTED A TREE");
							inPlace = true;
							rc.plantTree(dir);
						}
					}
	...
	static Direction moveAwayFrom(MapLocation ml, MapLocation pos) {
		return pos.directionTo(ml).opposite();
	}
}